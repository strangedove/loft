...
# Copyright 2025- Allura. All rights reserved.
#
...
if is_rich_available():
    from rich.console import Console #pyright: ignore[reportMissingImports, reportUnknownVariableType]
    from rich.panel import Panel #pyright: ignore[reportMissingImports, reportUnknownVariableType]
    from rich.table import Table #pyright: ignore[reportMissingImports, reportUnknownVariableType]
    from rich.text import Text #pyright: ignore[reportMissingImports, reportUnknownVariableType]

if is_comet_available():
    import comet_ml #pyright: ignore[reportMissingImports, reportUnknownVariableType]
...
def generate_model_card(
    ...
    card = ModelCard.from_template(
        card_data,
        template_path=str(pkg_resources.files("loft").joinpath(f"templates/{template_file}")),
        base_model=base_model,
        model_name=model_name,
...
def shuffle_sequence_dict(seq_dict: dict[str, Sequence[Any] | None]) -> dict[str, Sequence[Any] | None]:
    """
    ...
    def permute(v: Sequence[Any] | None) -> Sequence[Any] | None:
        if v is None:
            return None
